...
for i in range(0,len1):
    for key in closest_counties:
        if counties_won_by_hillary_clinton[0][i] == key:
...
'''
Finding candidate who is most correlated and second most correlated to Hillary Clinton
'''
...
print "The most correlated candidate to Hillary Clinton is %s" %(candidate)
print
...
print "The second most correlated candidate to Hillary Clinton is %s" %(candidate2)
print 
...
plt.figure()
plt.boxplot(visual)
plt.xlabel('Candidate Who Is Most Correlated With Hillary Clinton')
plt.ylabel('Number of Counties Candidates Participated In')
plt.xticks([1], ['John Kasich'])
plt.show()
